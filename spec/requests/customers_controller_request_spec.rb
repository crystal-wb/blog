...
RSpec.describe "CustomersControllers", type: :request do
  ...
  describe 'get customer_path' do
    ...
    it 'redirects to the index path if the customer is invalid' do
      get customer_path(id: 5000)
      expect(response.status).to redirect_to customers_path
    end
  end
  describe 'get new_customer_path' do
    it 'renders the index :new template' do
      get new_customer_path
      expect(response.status).to eq(200)
    end
  end
  describe 'get edit_customer_path' do
    it 'renders the index :edit template' do
      customer = FactoryBot.create(:customer)
      get edit_customer_path(id: customer.id)
      expect(response.status).to eq(200)
    end
  end
  describe 'post customers_path' do
    it 'saves a new entry and redirects to the show path for the entry' do
      ...
      expect(response.status).to redirect_to customer_path(id: Customer.last.id)
    end
  end
  ...
  describe 'put customer_path with valid data' do
    it 'updates an entry and redirects to the show path for the customer' do
      ...
      put customer_path(customer.id), {customer: customer_attributes}
      customer.reload
